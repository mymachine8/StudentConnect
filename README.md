# Student Connect #
(A Demo portal for students and faculty)
...
The sample project is a web application with an AngularJs-based frontend and a Spring/Hibernate based backend.

On the frontend, these libraries where used (besides Angular): [lodash](https://lodash.com/) for functional data manipulation. The module system [require.js](http://requirejs.org/) was used to load frontend dependencies. The dependencies where  obtained via [bower](http://bower.io/).
...
The passwords are not stored in the database in plain text but in a digested form, using the Spring Security [Bcrypt](http://docs.spring.io/autorepo/docs/spring-security/3.2.0.RELEASE/apidocs/org/springframework/security/crypto/bcrypt/BCryptPasswordEncoder.html) password encoder (transparently includes a salt).
...
Url           |Verb          | Description
--------------|------------- | -------------
/courses         |GET          | Display the courses of that user
/user| PUT| Update the details
